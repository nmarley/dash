...
public:
     ...
     * a spork to be considered active. This value must be at least a majority
     * of the total number of spork keys, and for obvious resons cannot be
     * larger than that number.
     */
    bool SetMinSporkKeys(int minSporkKeys);

    /**
     * SetPrivKey is used when the -sporkkey argument is given to DashCore, to
     * enable setting of spork values. This will return false if the private key
     * does not match the spork address.
     */
